...
import '../App.css'; // Make sure to import the CSS for animation
    ...
    const skillsData = [
        {
            ...
            textColor: 'text-blue-800 dark:text-violet-200/100',
        },
        {
                                    ...
                                    <div key={catIndex}>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {category.skills.map((skillName, skillIndex) => (
                                                <span key={skillIndex} className="relative rounded-full text-sm z-0 overflow-hidden">
                                                    {/* Animated border */}
                                                    <span className="absolute inset-0 rounded-full opacity-50 animate-animatedBorder border-gradient"></span>
                                                    {/* Inner content */}
                                                    <span className={`relative px-3 py-1 rounded-full ${skill.bgColor} ${skill.textColor} z-10`}>
                                                        {skillName}
                                                    </span>
                                                </span>
                                            ))}
